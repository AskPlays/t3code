  ...
  ServerConfig.layerTest(process.cwd(), { prefix: input.prefix }).pipe(
    ...
    Layer.provideMerge(
      Layer.succeed(HostProcessEnvironment, {
        ...
        // Isolate the fork's OpenCode sqlite scan: without this the service
        // reads the developer's real ~/.local/share/opencode database.
        XDG_DATA_HOME: NodePath.join(input.home, ".local", "share"),
        ...input.environment,
      }),
    ),
  );
