...
export const make = Effect.gen(function* () {
  ...
  const resolveTranscriptDirs = Effect.fn("UsageService.resolveTranscriptDirs")(function* (
    ...
    // Fork addition: OpenCode usage lives in sqlite, not jsonl transcripts.
    // XDG_DATA_HOME is read from the provider environment first so tests can
    // isolate it; otherwise the scan would read the developer's real
    // ~/.local/share/opencode database.
    const xdgDataHome =
      hostEnvironment["XDG_DATA_HOME"]?.trim() || process.env.XDG_DATA_HOME?.trim() || "";
    const openCodeDataDir = path.join(
      xdgDataHome || path.join(NodeOS.homedir(), ".local", "share"),
      "opencode",
    );
    dirs.push({
      provider: "opencode",
  ...
  interface ScannedDir {
    ...
    readonly kind: "jsonl" | "opencodeSqlite";
  ...
  const collectDirs = Effect.fn("UsageService.collectDirs")(function* (
    ...
    for (const entry of dirs) {
      const { provider, dir } = entry;
      const fileName = entry.kind === "jsonl" ? entry.fileName : undefined;
      const kind = entry.kind;
      const databasePath = entry.kind === "opencodeSqlite" ? entry.databasePath : undefined;
      const volumeId = yield* Effect.promise(() => readDirectoryVolumeId(dir));
      const exists = yield* fileSystem
        .exists(kind === "jsonl" ? dir : (databasePath ?? dir))
      ...
      if (kind === "opencodeSqlite") {
        // OpenCode usage is read from sqlite in the scan loop; files stays an
        // empty placeholder so the missing check above is the only gate here.
        scanned.push({
          provider,
          dir,
          volumeId,
      ...
      scanned.push({ provider, dir, volumeId, kind, files: parsedFiles });
    }
    return scanned;
  });
  ...
  const scanSummary = Effect.fn("UsageService.scanSummary")(function* (
    ...
    for (const scanned of scannedDirs) {
      const { provider, dir, volumeId, files } = scanned;
      if (files === null) {
        sources.push({
      ...
      if (scanned.kind === "opencodeSqlite") {
        const databasePath = scanned.databasePath ?? dir;
