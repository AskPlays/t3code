...
it.layer(OpenCodeAdapterTestLayer)("OpenCodeAdapterLive", (it) => {
  ...
  it.effect("ignores delayed stop events around the next turn startup", () =>
    Effect.gen(function* () {
      ...
      const eventsFiber = yield* adapter.streamEvents.pipe(
        ...
        // interrupt's abort + second turn.started + second turn.completed.
        Stream.take(6),
        Stream.runCollect,
        Effect.forkChild,
      );
  ...
  it.effect("uses the plan agent when interaction mode selects it without an agent option", () =>
    Effect.gen(function* () {
      const adapter = yield* OpenCodeAdapter;
  ...
  it.effect("keeps an explicit agent option across interaction modes", () =>
    Effect.gen(function* () {
      const adapter = yield* OpenCodeAdapter;
