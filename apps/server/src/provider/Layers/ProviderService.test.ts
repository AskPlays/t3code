...
function makeFakeCodexAdapter(
  ...
  options?:
    | {
        readonly getCommandCatalog?: ProviderAdapterShape<ProviderAdapterError>["getCommandCatalog"];
        readonly supportsConversationRollback?: boolean;
      }
    | boolean,
) {
  const normalizedOptions =
    typeof options === "boolean" ? { supportsConversationRollback: options } : options;
  const supportsConversationRollback = normalizedOptions?.supportsConversationRollback;
  ...
  const adapter: ProviderAdapterShape<ProviderAdapterError> = {
    ...
    stopAll,
    ...(normalizedOptions?.getCommandCatalog !== undefined
      ? { getCommandCatalog: normalizedOptions.getCommandCatalog }
      : {}),
    get streamEvents() {
