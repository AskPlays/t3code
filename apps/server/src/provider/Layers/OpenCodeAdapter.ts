...
interface OpenCodeCancellation {
  ...
  /**
   * A detached slash-command rejection that raced an in-flight interrupt.
   * The interrupt path surfaces it as the turn's terminal failure once the
   * abort settles, so the command failure is never lost or double-reported.
   */
  pendingCommandFailure?: {
    readonly turnId: TurnId;
    readonly error: ProviderAdapterRequestError;
  };
}
...
export function makeOpenCodeAdapter(
  ...
  return Effect.gen(function* () {
    ...
    /**
     * Terminal failure for a detached slash command whose endpoint rejected
     * outside any interruption. Mirrors the session.error shape: the turn
     * closes as failed and the session carries the error detail.
     */
     ...
     * owned nor parented stay dropped, as before. Returns whether the event
     * was consumed as a subagent event.
     */
    ...
    const handleSubscribedEvent = Effect.fn("handleSubscribedEvent")(function* (
      ...
      if (!isParentEvent && payloadSessionId !== undefined) {
        ...
        // task.* lifecycle; consumed events never reach the owned-session
        // path below. Unmatched types (e.g. child permission asks) keep
        // flowing through the child-request routing.
        if (yield* handleSubagentEvent(context, event, payloadSessionId)) {
          return;
        }
      }
    ...
    const sendTurn: OpenCodeAdapterShape["sendTurn"] = Effect.fn("sendTurn")(function* (input) {
      ...
      return yield* context.promptSemaphore.withPermit(
        Effect.gen(function* () {
          ...
          if (command) {
            // OpenCode's command endpoint does not return until the full
            // assistant turn finishes, so the request is detached: keep it
            // attached to the session scope and return the dispatch result
            // immediately — the event stream remains authoritative for turn
            // completion.
            context.promptAdmission = undefined;
            yield* runOpenCodeSdk("session.command", () =>
              ...
              Effect.tapError((requestError) =>
                Effect.gen(function* () {
                  ...
                  // An interruption owns this turn's terminal transition; a
                  // late or concurrent command rejection must never overwrite
                  // it.
                  if (context.interruptedTurnId === turnId) {
                    return;
                  }
                  const pendingCancellation = context.cancellation;
                  if (pendingCancellation?.turnId === turnId) {
                    // Interrupt in flight: hand the failure to the interrupt
                    // path, which surfaces it once the abort settles.
                    pendingCancellation.pendingCommandFailure = { turnId, error: requestError };
                    return;
                  }
                  yield* failDetachedCommand(context, turnId, requestError);
                }),
              ),
              Effect.ignore,
              Effect.forkIn(context.sessionScope),
            ...
            return {
              ...
              // binding is refreshed alongside last-seen/runtime state.
    ...
    const interruptTurn: OpenCodeAdapterShape["interruptTurn"] = Effect.fn("interruptTurn")(
      function* (threadId, turnId) {
        ...
        if (failedExit !== undefined && Exit.isFailure(failedExit)) {
          ...
          // A detached command failed while this interrupt was in flight and
          // the abort itself failed: the command failure is the turn's
          // terminal cause, so surface it before propagating the abort error.
          const commandFailure = cancellation.pendingCommandFailure;
          if (commandFailure && context.activeTurnId === commandFailure.turnId) {
            yield* failDetachedCommand(context, commandFailure.turnId, commandFailure.error);
          }
